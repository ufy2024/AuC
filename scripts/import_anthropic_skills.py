#!/usr/bin/env python3
"""从 https://github.com/anthropics/skills 导入官方 Agent Skills。

用法:
  python scripts/import_anthropic_skills.py
  python scripts/import_anthropic_skills.py --dry-run
"""
...
MANIFEST = ROOT / "auc" / "skill_library" / "bundled" / "_anthropics_manifest.json"

sys.path.insert(0, str(ROOT))

from auc.skill_library.anthropics_loader import discover_anthropics_skill_paths, enrich_anthropics_skill
from auc.skill_library.sources import ANTHROPICS_SKILLS_BRANCH, ANTHROPICS_SKILLS_REPO, ANTHROPICS_SKILLS_URL
from auc.skills import slugify


def fetch_repo_zip() -> Path:
    url = f"https://github.com/{ANTHROPICS_SKILLS_REPO}/archive/refs/heads/{ANTHROPICS_SKILLS_BRANCH}.zip"
    req = urllib.request.Request(url, headers={"User-Agent": "ufy-auc-import"})
    tmp = tempfile.NamedTemporaryFile(suffix=".zip", delete=False)
    with urllib.request.urlopen(req, timeout=180) as resp:  # noqa: S310
        tmp.write(resp.read())
    tmp.close()
    extract = Path(tempfile.mkdtemp(prefix="anthropics-skills-"))
...
def import_skills(*, dry_run: bool = False) -> dict[str, object]:
    ...
    paths = discover_anthropics_skill_paths(repo_root)
    ...
    for src in paths:
        ...
        try:
            ...
            skipped.append(rel)
            continue
        enriched = enrich_anthropics_skill(
            raw,
            rel_path=rel,
            source_url=f"{ANTHROPICS_SKILLS_URL}/blob/{ANTHROPICS_SKILLS_BRANCH}/{rel}",
        )
        if not enriched:
            skipped.append(rel)
            continue
        ...
        name = orig_name
        slug = slugify(name)
        # 与已有 bundled 技能冲突时加 anthropic- 前缀
        if slug in existing or slug in used_slugs:
            slug = slugify(f"anthropic-{name}")[:48]
            name = slug
    ...
    manifest = {
        "source": ANTHROPICS_SKILLS_URL,
        "branch": ANTHROPICS_SKILLS_BRANCH,
        "count": len(imported),
        "skills": imported,
        "skipped": skipped,
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Import anthropics/skills into AuC bundled library")
    ...
    print(f"Imported {manifest['count']} skills from {ANTHROPICS_SKILLS_URL}")
