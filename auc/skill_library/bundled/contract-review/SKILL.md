...
description: Contract review skill that adds comment-based issue annotations without
  changing original text. Enforces a three-layer review (basic, business, legal),
  writes structured comments (issue type, risk reason, revision suggestion) with risk
  level encoded via reviewer name, and generates a contract summary, consolidated
  opinion, and Mermaid business flowchart (with rendered image). Output language must
...
source: bundled
builtin: true
source_url: https://github.com/anbeime/skill/blob/main/skills/legal-assistant-skills-main/contract-review/SKILL.md
