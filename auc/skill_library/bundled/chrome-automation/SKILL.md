...
description: Connect to and control Google Chrome browser using agent-browser with
  CDP (Chrome DevTools Protocol). Use when the user wants to automate their existing
  Chrome browser, see browser actions in real-time, or needs to control the Chrome
  instance they're already using. Handles installation, setup, connecting via remote
  debugging, and all browser automation tasks with live visual feedback.
triggers:
- chrome-automation
- connect
- control
...
source: bundled
builtin: true
source_url: https://github.com/anbeime/skill/blob/main/skills/chrome-automation/chrome-automation/SKILL.md
division: operations
emoji: 🌐
roles:
